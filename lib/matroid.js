...
class Matroid {
  constructor(opts) {
    ...
    this.baseUrl = options.baseUrl || 'https://www.matroid.com/api/0.1';
    ...
    this.endpoints = {
      ...
      createFeed: this._makeEndpoint('POST', 'feeds'),
      monitorFeed: this._makeEndpoint('POST', 'feeds/:feed_id/monitor/:detector_id')
    };
  ...
  */
  ...
  monitorFeed(feedId, detectorId, configs) {
    return new Promise((resolve, reject) => {
      let options = {
        uriParams: {
          ':feed_id': feedId,
  ...
  _setAuthToken(callback, token) {
    if (!token || !token.token_type || !token.access_token) throw new Error(`Unable to extract token from ${token}`);
