...
const addEndPoints = (matroid) => {
  matroid.endpoints = {
    ...
    // detector feedback
    addFeedback: matroid._makeEndpoint(
      'POST',
      'detectors/:detectorId/feedback'
    ),
    deleteFeedback: matroid._makeEndpoint(
      'DELETE',
      'detectors/:detectorId/feedback/:feedbackId'
    ),
    ...
    watchMonitoringResult: matroid._makeEndpoint(
      'GET',
      'monitorings/:key/watch'
    ),
    ...
    getVideoSummaryTracks: matroid._makeEndpoint(
      'GET',
      'summaries/:summaryId/tracks.csv'
    ),
    getVideoSummaryFile: matroid._makeEndpoint(
      'GET',
      'summaries/:summaryId/video.mp4'
    ),
    ...
    getStreamSummaries: matroid._makeEndpoint(
      'GET',
      'streams/:streamId/summaries'
    ),
    createStreamSummary: matroid._makeEndpoint(
      'POST',
...
const addHelpers = (matroid) => {
  matroid._genericRequest = function (options, resolve, reject) {
    let { action, data, filePaths, uriParams, shouldNotParse, noAuth } =
      options;
  ...
  matroid._checkFilePayloadSize = function (
    filePaths,
    batchLimit,
    singleLimit
  ) {
    ...
    if (
      ...
      throw new Error(
        `Individual file size must be under ${
          this._fileSizeLimits.image / 1024 / 1024
        }MB; Batch size under ${
          this._fileSizeLimits.imageBatch / 1024 / 1024
        }MB`
      );
    }
  };
...
function validateMediaObj(media, mediaType) {
  if (
    !media ||
    (typeof media.url === 'undefined' && typeof media.file === 'undefined')
  ) {
